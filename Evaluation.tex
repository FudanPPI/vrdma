...
Graph500~\cite{Graph500} is a rating system for data intensive applications in HPC. The banchmark used in Graph500 will do the following tasks: generating graphs according to the input parameters, performing the breadth-first search (BFS) and single source shortest path (SSSP). 

We run BFS and SSSP banchmark on two servers with 16 MPI processes. The banchmake generates graphs in the main memory with parameters scale \= 26 and edge\_factor \= 16. We run 16 times for each test and calculate the horizontal mean of traversed edges per second (TEPS). As the data shown in Figure~\ref{fig:graph500}, the performance of \sys is very close to SR-IOV and \native.
