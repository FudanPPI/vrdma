...
% RDMA网卡具备数据零拷贝，绕过内核以及硬件协议栈等特点，因此具备了高性能。具体来讲，RDMA中控制路径和数据路径分离，控制路径上，RDMA网卡会按上层应用的命令创建RDMA资源并维护RDMA上下文，例如，RDMA网卡会在创建资源时缓存资源的key，ID及注册内存的页表等信息；在数据路径上，RDMA网卡依照RDMA资源中的地址信息以零拷贝的方式读写数据，并通过网卡硬件协议栈进行收发，减少了CPU的消耗。此外，对于用户空间的RDMA应用，RDMA网卡在数据路径绕过了操作系统，直接根据缓存的页表读写用户空间的内存。

% RDMA有多种不同的实现方式，例如，InfiniBand，RoCE和iWarp。不过，Verbs是上层应用使用RDMA网卡的基本接口。接下面英文：
