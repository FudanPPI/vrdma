...
% vRNIC作为虚拟设备，是动态分配给各虚拟机或容器的，而且各vRNIC需要共享底层物理RDMA网卡。为了对vRNIC进行统一管理，促进物理RDMA网卡资源的高效利用，以及实现各vRNIC之间的虚拟RDMA网络，我们在各主机服务器上构建了一个统一的虚拟层URN core。URN core的主要功能包括实例化vRNIC和配置虚拟RDMA网络等。



% 4.2.1 实例化vRNIC 

% 首先，在实例化vRNIC时，URN Core需要对vRNIC的不变属性进行初始化。vRNIC中的不变属性包括RDMA地址vGID，设备号等等。对于RDMA地址vGID，URN core会通过mgmt center进行协调和登记，以确保各vRNIC的RDMA地址等信息是不冲突的并维护在统一的数据库中。由于vRNIC后端接受的RDMA命令，最终仍需要基于物理RDMA网卡执行。因此，实例化vRNIC后，仍需与对应的RDMA物理网卡设备完成绑定。如果是多网卡设备或者多个网卡接口（如使用SR-IOV），还需确定从vRNIC到对应物理网卡接口的映射关系。
...
%在云环境中，虚拟网络配置是实现很多功能的基础，例如多租户隔离、便携的虚拟机或容器迁移，因为容器和虚拟机迁移后无需更改物理网卡及交换机等硬件的网络配置。虚拟网络配置包括了虚拟的网络地址，虚拟的路由规则等。

% 首先，URN core 在实例化vRNIC时， 给vRNIC配置的vGID是虚拟值，与物理网卡的GID无关。

% 其次，每台主机的URN core中配置了虚拟RDMA网络路由规则。具体来说，URN core在实例化vRNIC时，除了硬件属性以外，还给每个vRNIC都配置了组ID。 路由规则的定义形式为:{group ID1, group ID2, Policy}。例如，如图4所示，容器1和容器2的vrnic被配置到同一组中，因此允许创建RDMA连接。作为对比，容器和虚拟机1是隔离的因为属于不同组。建立连接的具体细节可以参考4.3.1中的第（3）节。
