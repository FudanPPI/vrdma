...
% 4.1， vRNIC 设计
%   vRNIC 由前后端组成， 前端由xxx组成， 后端xxx，维持了虚拟和物理的RDMA上下文及其映射关系。控制路径xxx，经过前后端， 数据路径xxx，绕过前后端：【该部分在overview里面已经介绍过了？如何厘清Overview和Design的边界？】

%  4.1.1 用户空间的vRNIC后端
%  vRNIC后端的主要功能？
%  vRNIC后端的功能主要有两个：
%  1， 接受和执行vRNIC前端传递的RDMA控制命令，并返回给Guest中的前端， 如create_qp 
%  2， 配合vRNIC前端，完成RDMA资源（如QP）和数据从后端到guest应用程序的映射。

%  vRNIC后端实现在用户空间的好处？
%  在URN中，vRNIC 后端实现在主机的用户空间作为一个虚拟设备。在用户空间主要有三方面的优势：
%  1， 用户空间更适合管理：开发管理功能，在用户空间更加简单并且灵活；
%  2，用户空间的稳定性更好：由于用户空间不存在面向内核的攻击面；
%  3，用户空间的兼容性更好，因为用户空间对于不同的系统架构、内核版本和设备内核驱动兼容.

%  vRNIC后端实现在用户空间的挑战和解决方式？
%  vRNIC后端的挑战是在主机用户空间完成上述的两个功能：1， 执行guest转发的RDMA命令；2，完成RDMA资源映射。因为用户空间的vRNIC后端本质上仍然是主机端的RDMA应用，没法直接利用RDMA内核软件栈中的API以及主机操作系统的内存管理。为了解决这一问题： 
% 为了实现功能1，对于虚拟机，我们需要将虚拟机内核发出的RDMA命令和参数转化为verbs的命令和参数，对于容器，直接修改容器的verbs用户库，并进行转发；
% 为了实现功能2， 利用了共享内存的系统调用，以与guest的RDMA资源使用相同的内存物理页面。为此，对于虚拟机：在虚拟机启动时，vRNIC后端即与虚拟机完成指定内存的共享，虚拟机创建RDMA资源时仅使用对应的与vRNIC后端共享的物理内存；对于容器：在容器应用创建RDMA资源时，会修改verbs库以使用共享内存创建资源，并将资源的共享路径传递给vRNIC后端。 因此，对于vRNIC后端，使用修改的verbs库，使用对应的共享内存去创建资源，便可以完成对虚拟机或容器应用的资源映射。
