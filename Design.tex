...
% 4.1.1 对于vRNIC后端，我们将其设计在主机用户空间，用户空间有诸多好处，细节可以见2.1节。在vRNIC后端中，需要维护vRNIC的两方面属性，以向guest应用提供完整的RDMA功能：

% 其一，静态属性，是vRNIC所具备的成员或属性。vRNIC的静态属性主要包括：1）RDMA地址等硬件信息，供上层guest查询；2）RDMA资源缓冲区，如QP，MR，CQ或MR等，用以存储虚拟的RDMA资源实例或其信息等。只有vRNIC具备了这两类静态属性，经过实例化后，guest才能够使用vRNIC以实现RDMA。由于vRNIC后端是由软件实现的，其静态属性可以完全使用任意的虚拟值，vRNIC的静态属性与物理RDMA设备解耦。对于RDMA地址的虚拟化，有助于实现RDMA网络虚拟化，也方便了后续虚拟实例的迁移。因为，vRNIC向guest暴露的RDMA地址是虚拟地址，RDMA应用对底层的物理RDMA地址配置无感知。对于RDMA资源缓冲区的虚拟化，方便了对各虚拟实例使用RDMA资源进行限制与管理。例如，vRNIC在实例化时，可以限制vRNIC资源buffer的大小，以限制某一guest对RDMA资源的使用情况。

% 其二，动态属性，是vRNIC所支持的操作过程。vRNIC可以支持RDMA的控制路径和数据路径， 两者均与具体的RDMA资源实例相关，例如控制路径主要是QP、MR等RDMA资源实例的创建、 更新或销毁，vRNIC后端需要随时记录和变更缓冲区中的资源实例信息，包括key，ID及页表信息等；数据路径主要是RDMA资源的直接使用。虚拟的RDMA资源实例最终仍需反映到物理RDMA网卡上，即每次对虚拟RDMA资源实例的操作均需要反映到对应的真实RDMA资源上，以实现RDMA功能。此外，在RDMA的上下文中，各RDMA资源之间具备复杂的交互关系，例如QP和CQ之间具备直接关联，MR和QP等也具备间接关联。因此，vRNIC后端中为了具备完整的动态属性，需要维护维护三方面的信息：
% 1，来自guest的虚拟RDMA资源实例，这部分虚拟RDMA资源实例或信息主要存储与上述静态属性的资源缓冲区中；
% 2，由物理网卡创建的RDMA资源实例，这部分由vRNIC后端利用verbs库或系统调用等在用户空间维护，各RDMA资源实例之间构成真实的RDMA上下文；
% 3，虚拟实例到真实实例之间的对应关系，注意，这个对应关系是一对一关联，用以确保各虚拟RDMA上下文中，各虚拟RDMA资源之间的交互关系是严格与真实的RDMA上下文相吻合的，确保不会出现错误。

% 完成了上面两种属性的虚拟化之后，vRNIC具备了基本的RDMA功能，可以为上层的guest提供RDMA服务。

% 4.1.2 vRNIC前端
% 对于vRNIC前端，在虚拟机和容器中有不同的vRNIC前端，这主要是因为虚拟机中的RDMA应用与主机空间之间隔着虚拟机操作系统，而容器内RDMA应用仍由主机操作系统管理。 
% 在虚拟机中，为了最大限度地实现对上层应用的透明性，我们将vRNIC前端设计在虚拟机内核中，拦截RDMA内核驱动的RDMA命令，而不是用户空间verbs库。这样做的好处是，vRNIC前端在虚拟机中既支持原生的用户空间Verbs API也能支持内核驱动的与设备无关的API。为此，虚拟机中的vRNIC前端需要配合一个自定义的设备相关的驱动模块，该模块用来欺骗RDMA内核驱动与设备无关的OFED模块，注册模拟设备的相关函数到OFED驱动模块中。RDMA应用在控制路径上的的verbs命令经过Verbs库的系统调用后，到达OFED驱动，从而调用自定义设备相关模块的接口。在该接口中，来自guest用户空间的verbs命令会转化成guest内核OFED驱动的RDMA命令。vRNIC前端会将该命令转发到vRNIC后端。不过考虑到，vRNIC后端在主机用户空间，且使用主机的Verbs库接口。因此，guest中vRNIC前端在转发命令到后端前，仍需要将内核的RDMA命令和参数转化为序列化的Verbs的RDMA命令和参数形式，以让vRNIC后端调用。
% 在容器中，我们尽可能在用户空间完成vRNIC前端转发verbs命令的功能，因此，我们修改了容器的verbs库中与内核RDMA驱动交互的接口，将其替换为vRNIC前端接口。即vers库中与内核交互的RDMA命令及参数，并被劫持到vRNIC前端。进一步，通过vRNIC前端直接转发到vRNIC后端，无需对命令和参数进行转化。

% 4.1.3 映射优化
% RDMA命令中的控制路径和数据路径分离，控制路径包括创建RDMA资源和建立RDMA连接，在RDMA应用中往往是一次性操作；而数据路径与数据收发操作相关，是影响RDMA网络性能的关键。因此，对于RDMA命令的虚拟化方式，我们继承了控制和数据通道分离的哲学，和已有的RDMA虚拟化方案一样，如hyv和MasQ。 在控制路径上： guest应用在本地的虚假RDMA上下文中执行verbs命令，例如create_qp, 并经过vRNIC前端转发到后端维持的真实RDMA上下文中执行，之后将结果返回给guest。在数据路径上，guest应用直接使用映射的RDMA资源，guest的verbs命令直接在本地执行即可，从而绕过了vRNIC的前后端。 要实现上述效果，需要在guest应用和vRNIC后端之间完成各RDMA资源的映射。

% RDMA资源按照位于的物理地址空间，分为内存资源（如QP）和设备资源（如门铃）。这些RDMA资源的映射操作，均通过vRNIC的前后端来完成。
% 对于内存资源（如QP），对于虚拟机：在虚拟机启动时，vRNIC后端即与虚拟机完成指定内存的共享，虚拟机创建RDMA资源时仅使用对应的与vRNIC后端共享的物理内存；对于容器：在容器应用创建RDMA资源时，会修改verbs库以使用共享内存创建资源，并将资源的共享路径传递给vRNIC后端。因此，对于vRNIC后端，使用修改的verbs库，使用对应的共享内存去创建资源，便可以完成对虚拟机或容器应用的资源映射。
% 设备资源（如门铃），位于设备地址空间，由RDMA设备驱动管理。因此，无法在用户空间完成映射。为此，我们设计了一个用于映射RDMA设备资源且与host端RDMA内核驱动解耦的内核模块。vRNIC前端会将设备资源映射的命令和地址参数，转发到一个host内核模块，由该模块完成映射操作。


% 4.2  URN Core 设计
    % 在每台主机上，URN core设计的目的是为了管理服务器上的虚拟或物理RDMA资源，因此，URN core的功能主要有两个：1，实例化各vRNIC后端； 2，创建虚拟的RDMA连接.

% 4.2.1 vRNIC实例化及分配

% vRNIC实例化由URN core进行，在实例化vRNIC时，需要对vRNIC的静态属性进行初始化。对于RDMA地址等硬件信息，URN core会通过mgmt core进行协调和登记，以确保各vRNIC的RDMA地址等信息是不冲突的并维护在统一的数据库中。对于各RDMA资源缓冲区，其初始化时各RDMA资源实例都为空，即还未创建状态。vRNIC在何时进行实例化，分为动态和静态等实例化模式。对于静态模式，由URN core先对vRNIC进行实例化，待接受来自guest的vRNIC请求后，立即将该vRNIC实例分配给对应guest； 对于动态模式，则是在接受来自guest的vRNIC请求后，再进行实例化并分配给guest。通常来说，可以使用动态模式，避免了vRNIC实例的冗余，节省了内存等不必要开销。


% vRNIC在实例化后，需要考虑如何分配给各个guest，为此，URN core与虚拟机和容器之间建立了一个消息通道，以接受虚拟机和容器探测vRNIC设备的请求，完成vRNIC实例与guest之间的绑定。当guest需要vRNIC时，他们可以请求URN core在主机用户空间中实例化相应的后端。为了构建这个通道，我们使用了UNIX套接字对于虚拟机及容器环境。当虚拟机或容器需要一个新的vRNIC时，对套接字发出的实例化请求会被URN处理。为了快速支持多个实例化请求，这个过程中采用了反应器模式。
