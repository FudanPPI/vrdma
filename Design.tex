...
% 4.1.1 vRNIC后端
% 对于vRNIC后端，我们将其设计在主机用户空间，用户空间有诸多好处，细节可以见2.1节。在vRNIC后端中，需要维护vRNIC的两方面属性，以向guest应用提供完整的RDMA功能：
...
% 完成了上面两种属性的虚拟化之后，vRNIC可以作为一个软件的虚拟RDMA设备，可以为上层的guest提供RDMA服务。
...
% 4.1.3 映射优化
% RDMA的操作过程分为控制路径和数据路径：控制路径包括创建RDMA资源和建立RDMA连接，一般在连接初始化阶段完成，不影响传输过程；而数据路径与数据收发操作相关，每次数据的传输过程中都会涉及，是影响RDMA网络性能的关键。因此，对于RDMA命令的虚拟化方式，我们继承了控制和数据通道分离的哲学，和已有的RDMA虚拟化方案一样，如hyv和MasQ。 在控制路径上： guest应用在本地的虚假RDMA上下文中执行verbs命令，创建RDMA资源或建立连接，例如create_qp, 并经过vRNIC前端转发到后端维持的真实RDMA上下文中执行，之后将结果返回给guest。在数据路径上，guest应用使用虚拟RDMA资源，guest的verbs命令直接在本地执行即可，从而绕过了vRNIC的前后端。 

% 要实现上述功能，需要将guest中控制和数据命令、参数等拷贝到vRNIC后端，由vRNIC后端在真实的RDMA上下文中执行，然后将结果拷贝到vRNIC前端。这种方式比较简单，但是导致额外的数据拷贝，从而影响RDMA性能。为了解决这个问题，对RDMA资源进行了直接映射的方式。
