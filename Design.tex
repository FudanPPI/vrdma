...
% 【这段只有中文】URN core 服务会在每台服务器上运行，统一监听该主机上所有虚拟机、容器和原生的获取RDMA设备的请求（如果原生的请求不进行统一处理，将会导致原生和虚拟化对RDMA设备接口资源的冲突，因此修改了主机打开设备的Verbs实现，将对应请求转发到URN core，由URN core统一分配RDMA设备接口）。一旦有新的获取RDMA设备请求到来，便会唤醒URN core服务监听的描述符。
...
% （2）对RDMA虚拟化扩展的支持：RDMA虚拟化中，需要对QP/CQ等RDMA资源进行资源映射，避免额外的数据拷贝及切换开销。现有的Verbs库和用户态驱动没有对RDMA资源映射的支持。为此，我们对Verbs库及其OFED内核驱动提出了下述建议，以加强对各场景下RDMA虚拟化的支持，让应用和虚拟层可以在用户空间便捷、高效地完成MR、QP/CQ等资源的映射。具体建议如下：

% 对于QP/CQ: 对于虚拟机应用，建议在用户态驱动中调用mlx4_alloc_buf分配QP/CQ的buffer时，增加使用共享内存的选择。对于虚拟层，在创建qp/cq资源的过程中，在用户态驱动调用alloc_assigned_buf，代替原有的mlx4_alloc_buf为QP/CQ资源分配指定内存位置的buffer。

% 对于MR: 对于虚拟机应用，增加重新映射。
% API  所属库  功能 
% 前端verbs库： mlx4_alloc_buf, 
% 
% 后端Verbs库： alloc_assigned_buf(shm-addr, size) 设备相关库 为RDMA资源指定某一内存区域,shm-addr 怎么来的？
