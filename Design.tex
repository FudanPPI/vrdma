...
% 对于vRNIC后端，我们将其设计在主机用户空间，用户空间有诸多好处，细节可以见2.1节。在vRNIC后端中，需要维护与RDMA网卡相关的两方面属性，以向guest应用提供完整的RDMA功能：其一，静态属性，该属性与RDMA上下文无关，例如设备名，RDMA地址等；其二，动态属性，该属性与运行时的RDMA上下文相关，例如QP、MR等RDMA资源，由RDMA应用创建、更新或销毁，RDMA网卡需要随时记录和变更资源元数据信息，包括key，ID及页表信息等。对于静态属性，对各guest应用都是通用的，guest应用无权限也无必要更改，因此，vRNIC后端中维护的静态属性可以完全使用虚拟值，并且在vRNIC实例化时进行初始化，与物理RDMA设备解耦；对于动态属性，该属性与RDMA应用的运行时过程相关，需要与物理RDMA网卡绑定，例如，上层guest应用再创建QP资源时，vRNIC后端也需相应地基于物理RDMA网卡创建QP资源。总之，对于动态属性，vRNIC后端中需要维护三方面的信息：1，虚拟机RDMA上下文信息；2，主机端物理RDMA上下文信息；3，两者的对应关系等。【RDMA地址是静态属性还是动态属性？】
...
% 4.2.1， URN core如何实例化vRNIC后端？ 
% vRNIC后端在实例化时，需要与虚拟机或容器之间协调以完成初始化工作。为此，URN core与虚拟机和容器之间建立了一个消息通道，以接受虚拟机和容器初始化的请求，完成vRNIC的实例化。当guest需要vRNIC时，他们可以请求URN core在主机用户空间中实例化相应的后端。为了构建这个通道，我们使用了UNIX套接字对于虚拟机及容器环境。当虚拟机或容器需要一个新的vRNIC时，对套接字发出的实例化请求会被URN处理。为了快速支持多个实例化请求，这个过程中采用了反应器模式。

% 4.2.2， URN core如何创建虚拟RDMA连接？
